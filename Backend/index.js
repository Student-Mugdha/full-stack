  ...
  db.query(
    ...
    (err, cartResult) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "An error occurred while creating the cart." });
      }
    }
  );
});
...
app.post("/addproduct", (req, res) => {
  ...
  db.query(imageQuery, [productName], (err, imageResults) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Error fetching image URL", details: err.message });
    }
    ...
    db.query(checkQuery, [productName, vendorID], (err, results) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Error checking product", details: err.message });
      }
...
app.post("/subscription", (req, res) => {
  ...
  const query =
    "INSERT INTO Subscription (subs_Type, StartDate, EndDate,userID) VALUES (?, ?, ?, ?)";
  db.query(query, [subs_Type, StartDate, EndDate, userID], (error, results) => {
    if (error) {
...
app.post("/reviews", (req, res) => {
  ...
  // Prepare SQL query
  const sql =
    "INSERT INTO reviews (userName, userEmail, rating, review) VALUES (?, ?, ?, ?)";
